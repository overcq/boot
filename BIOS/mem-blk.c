...
_internal P E_mem_Q_blk_R_new_0( N * );
//==============================================================================
void
...
){  E_base_S->E_mem_Q_blk_Q_table_M_from_free_or_map_0_S_allocated_id[ E_base_S->E_mem_Q_blk_Q_table_M_from_free_or_map_0_S_allocated_id_n++ ] = allocated_or_table_i;
}
_internal
void
E_mem_Q_blk_Q_table_I_put_before( N table_i
){  E_base_S->E_mem_Q_blk_Q_table_M_from_free_or_map_0_S_table_id[ E_base_S->E_mem_Q_blk_Q_table_M_from_free_or_map_0_S_allocated_id_n - 1 ] = table_i;
}
_internal
void
E_mem_Q_blk_Q_table_I_put_after( N table_i
){  for_n( i, E_base_S->E_mem_Q_blk_Q_table_M_from_free_or_map_0_S_allocated_id_n )
    {   N *allocated_or_table_i = E_base_S->E_mem_Q_blk_Q_table_M_from_free_or_map_0_S_allocated_id[i];
        if( E_base_S->E_mem_Q_blk_Q_table_M_from_free_or_map_0_S_table_id[ E_base_S->E_mem_Q_blk_Q_table_M_from_free_or_map_0_S_allocated_id_n - 1 ] < *allocated_or_table_i
        && table_i >= *allocated_or_table_i
        )
            (*allocated_or_table_i)--;
        else if( E_base_S->E_mem_Q_blk_Q_table_M_from_free_or_map_0_S_table_id[ E_base_S->E_mem_Q_blk_Q_table_M_from_free_or_map_0_S_allocated_id_n - 1 ] > *allocated_or_table_i
        && table_i <= *allocated_or_table_i
        )
            (*allocated_or_table_i)++;
    }
...
){  E_base_S->E_mem_Q_blk_Q_table_M_from_free_or_map_0_S_allocated_id_n--;
}
...
, N align
    ...
    {   if( *allocated_or_table_i == E_base_S->E_mem_Q_blk_S_free_id
        ...
        || *allocated_or_table_i == E_base_S->E_mem_Q_blk_S_free_id
            ...
            l_align = 1;
        if( *allocated_or_table_i == E_base_S->E_mem_Q_blk_S_free_id ) // Obszar przed wyrównanym adresem staje się wolnym blokiem (1 u) oraz dla ewentualnego ‘mapowania’ tablicy zmapowanych bloków (2 u).
        {   n += 3;
            l_1 += 3 * u;
        }
        N l_ = ~0;
    ...
    {   N allocated_i_sorted_pos;
        if( !( p_1 = E_mem_Q_blk_R_new_0( &allocated_i_sorted_pos )))
            return 0;
        E_base_S->E_mem_Q_blk_S_allocated[ *allocated_or_table_i ].p = p_1;
...
P
E_mem_Q_blk_R_new_0( N *allocated_i_sorted_pos
...
, N align
    ...
    N allocated_i = E_mem_Q_blk_Q_sys_table_R_new_id( E_base_S->E_mem_Q_blk_S_allocated_id, (Pc)&allocated_p.p - (Pc)&allocated_p, (Pc)&allocated_p.n - (Pc)&allocated_p, 0, 0 );
    if( !~allocated_i )
        return 0;
...
, N n
    ...
    O{  if( E_base_S->E_mem_Q_blk_S_allocated[ allocated_i ].p == *( P * )p )
        {   N l = n * E_base_S->E_mem_Q_blk_S_allocated[ allocated_i ].u;
            ...
            {   N allocated_i_sorted_pos;
                P p_ = E_mem_Q_blk_R_new_0( &allocated_i_sorted_pos );
                if( !p_ )
                    return 0;
                E_mem_Q_blk_Q_table_I_put_before( E_base_S->E_mem_Q_blk_S_free_id );
                struct E_mem_Q_blk_Z_free free_p_;
