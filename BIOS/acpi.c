...
E_acpi_I_rsdp( struct H_acpi_Z_rsdp *rsdp
    ...
    {   struct H_acpi_Z_rsdt *rsdt = E_main_Z_p_I_to_virtual_pre( (P)(N)rsdp->RSDT_address );
        ...
        {   struct H_acpi_Z_table_header *header = E_main_Z_p_I_to_virtual_pre( (P)(N)rsdt->table_address[ table_i ] );
            if( !header
            || header->length <= sizeof( *header )
            || E_acpi_I_checksum( header, header->length )
            )
                return ~0;
            if( E_mem_Q_blk_T_eq( &header->signature[0], "APIC", sizeof( rsdt->header.signature )))
            ...
            }else if( E_mem_Q_blk_T_eq( &header->signature[0], "DMAR", sizeof( rsdt->header.signature )))
            ...
            }else if( E_mem_Q_blk_T_eq( &header->signature[0], "FACP", sizeof( rsdt->header.signature )))
            {   P facs_physical;
                ...
                {   struct H_acpi_Z_fadt_v1 *fadt = (P)header;
                    ...
                    || !fadt->dsdt
                        ...
                        return ~0;
                    header = E_main_Z_p_I_to_virtual_pre( (P)(N)fadt->dsdt );
                    facs_physical = (P)(N)fadt->facs;
                }else
                    return ~0;
                struct H_acpi_Z_facs_v0 *facs = E_main_Z_p_I_to_virtual_pre( (P)facs_physical );
                if( !header
                || !facs
                )
                    return ~0;
                ...
                || E_acpi_I_checksum( header, header->length )
                || !E_mem_Q_blk_T_eq( &header->signature[0], "DSDT", sizeof( rsdt->header.signature ))
                )
                    return ~0;
                ...
                if( facs
                && ( !E_mem_Q_blk_T_eq( &facs->signature[0], "FACS", sizeof( facs->signature ))
                  || facs->length != sizeof( *facs )
                ))
                    return ~0;
                E_main_S_kernel_args.acpi.facs = facs_physical;
            }else if( E_mem_Q_blk_T_eq( &header->signature[0], "HPET", sizeof( rsdt->header.signature )))
            ...
            }else if( E_mem_Q_blk_T_eq( &header->signature[0], "MCFG", sizeof( rsdt->header.signature )))
            ...
            }else if( E_mem_Q_blk_T_eq( &header->signature[0], "SSDT", sizeof( rsdt->header.signature )))
            ...
            }else if( E_mem_Q_blk_T_eq( &header->signature[0], "WAET", sizeof( rsdt->header.signature )))
            ...
            }else if( E_mem_Q_blk_T_eq( &header->signature[0], "WSMT", sizeof( rsdt->header.signature )))
...
E_acpi_I_search( void
    ...
    {   E_main_S_kernel_args.local_apic_address = (P)0xfee00000;
        E_main_S_kernel_args.io_apic_address = (P)0xfec00000;
        r = 0;
    }
    return r;
}
